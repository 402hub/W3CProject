...
export class MessagingEngine {
  ...
  async sendMessage(conversationId, content, options = {}) {
    ...
    try {
      ...
      // 3. BACKGROUND: Send to XMTP (if online and client ready)
      if (this.isOnline && this.client) {
        try {
          ...
          throw new Error(`XMTP Error: ${xmtpError.message}`);
        }
  ...
  async getOrCreateConversation(peerAddress) {
    try {
      ...
      if (this.conversations.has(peerAddress)) {
        const cachedConv = this.conversations.get(peerAddress);
        if (cachedConv) {
          return cachedConv;
        }
      }
      ...
      if (!conversation) {
        throw new Error('Failed to create conversation - returned null');
      }
      ...
      console.log(`✅ Conversation created: ${peerAddress}`);
      return conversation;

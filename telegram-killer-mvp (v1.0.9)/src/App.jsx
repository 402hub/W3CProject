...
function MainApp() {
  ...
  async function initializeMessaging() {
    ...
    try {
      ...
      console.log('🔄 Initializing messaging engine...');
      
      // Initialize messaging engine with dev environment (more stable for testing)
      ...
      try {
        ...
        console.warn('⚠️ Payment engine failed (non-critical):', payError.message);
      }
      ...
      } else if (error.message.includes('V2') || error.message.includes('legacy')) {
        toast.error('XMTP version issue. Try refreshing the page.', { 
          duration: 5000 
        });
      } else {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      // Send message (messaging engine handles optimistic update)
      ...
      if (error.message.includes('V2') || error.message.includes('legacy')) {
        toast.error('XMTP V3 upgrade recommended', {
          duration: 5000
        });
      }
    }
